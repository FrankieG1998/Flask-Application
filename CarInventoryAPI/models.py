...
class Book(db.Model):
    ...
    book_type = db.Column(db.String(30))
    book_language = db.Column(db.String(50))
    book_edition = db.Column(db.String(50))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self,book_title,isbn,author_name,no_of_pages,book_type,book_language,book_edition,user_token, id = ''):
        self.id = self.set_id()
...
class BookSchema(ma.Schema):
    class Meta:
        fields = ['id', 'book_title','isbn','author_name', 'no_of_pages', 'book_type', 'book_language','book_edition']
